...
def restore_resolv_conf():
    if os.path.exists(RESOLV_CONF_BACKUP_PATH):
        ...
        print("Restored original resolv.conf")
...
def configure_resolv_conf():
    backup_resolv_conf()
    if (
        not os.path.exists(RESOLV_CONF_PATH)
        or RESOLV_CONF_ENTRY not in open(RESOLV_CONF_PATH).read()
    ):
...
def start_tor_service():
    ...
    if status != 0:
        print(
            "Failed to start TOR service. Please check the TOR logs for "
            "details."
        )
        exit(1)
    else:
...
def check_tor_status():
    ...
    service_active = (
        subprocess.run(
            f"systemctl is-active --quiet {TOR_SERVICE_NAME}",
            shell=True,
    ...
    print(f"TOR Service: {'ACTIVE' if service_active else 'INACTIVE'}")

    # Check if we have a TOR IP
    try:
        tor_ip = get_tor_ip()
        print(f"TOR IP: {tor_ip}")
    except Exception:
        print("TOR IP: Not available")

    # Check iptables for TOR redirection rules
    iptables_check = (
        subprocess.run(
            "iptables -t nat -C OUTPUT -p tcp --tcp-flags FIN,SYN,RST,ACK SYN "
    ...
    print(f"IP Redirection: {'ACTIVE' if iptables_check else 'INACTIVE'}")

    # Check resolv.conf configuration
    resolv_configured = False
    if os.path.exists(RESOLV_CONF_PATH):
        with open(RESOLV_CONF_PATH, "r") as f:
            if RESOLV_CONF_ENTRY.strip() in f.read():
                resolv_configured = True

    print(
        "DNS Configuration: "
        f"{'CONFIGURED' if resolv_configured else 'NOT CONFIGURED'}"
    )
    ...
    print(f"IPv6: {'DISABLED' if ipv6_disabled else 'ENABLED'}")
    print("-----------------\n")
...
def start_torward():
    ...
    # Make sure outputs are flushed immediately
    import sys
    ...
    print("Torward has been successfully started!")
...
def stop_torward():
    ...
    print("Torward has been successfully stopped!")
